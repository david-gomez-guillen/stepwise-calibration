...
### Hyperparameter tuning (lengthscales)

maxima <- c()
n.pars <- 11 * MAX.SIMULATION.MATRICES
n.ll.points <- 20
train.data <- lapply(initial.guess, 
...
for(i in seq(1,n.pars)) {
  x <- lapply(seq(n.pars), function(p) {
    10^seq(-12,0,length.out=n.ll.points)
  })
  ...
  for(xx in seq(5)) {
    ...
    for(j in seq_along(x[[i]])) {
      l <- x[[i]][j]
      err <- calculate.lengthscale.ll(l, i, 'oak.gaussian', train.data, f)
      err$x <- l
    ...
    # print(plt)
    lls <- sapply(errors, function(x)x$mean)
    # max.estimate.ind <- which(-lls > -lls[1]*10)[1]
    # max.estimate.ind <- which.min(diff(lls))
    max.estimate.ind <- which.max(lls)
    x[[i]] <- 10^seq(log10(errors[[max.estimate.ind-2]]$x),
                     log10(errors[[max.estimate.ind+2]]$x),
                     length.out=n.ll.points)
