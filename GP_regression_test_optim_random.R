...
### Kernel
k <- function(x,x2) 2*exp(-2*(x-x2)^2)
...
calculate.regression.model <- function(X, y) {
  ...
  sigma <- function(Xs) {
    ...
    Kss <- outer(Xs, Xs, k)
    S <- Kss - Ks %*% Ki %*% t(Ks)
    if (all(Xs %in% observed.x) && f.noise == 0)
...
choose.next.evaluation.points <- function(x, y.acq, observed.x, gp.model) {
  ...
  if (!use.dumb.optimization) {
    ...
    random.points <- c(randomLHS(random.intervals, 1)) * 10
    random.points <- sample(random.points)
    ys <- sapply(random.points, function(x) acq.func(gp.model, x))
    best.point <- random.points[which.max(ys)]
  }
  return(best.point)
}
...
for(n in seq(n.iterations)) {
  ...
  # print(plt2)
  # dev.off()
  cat(paste0('Iteration ',n,': ', gp.model$best.x, ' (', gp.model$best.y, ')\n'))
}
