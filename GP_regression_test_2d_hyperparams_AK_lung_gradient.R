...
fit.model <- function(pars, kernel.type) {
  if (kernel.type == 'se') {
    l <- pars[1]
    sigma2 <- pars[2]
  } else if (kernel.type == 'ak') {
...
calculate.params.ll <- function(pars, kernel.type) {
  ...
  gp.model <- fit.model(pars, kernel.type)
...
calculate.params.ll.gr <- function(pars, kernel.type) {
  start_time <- Sys.time()
  # perfs <- pbsapply(cl=cl, X=seq(n.iters.per.paramset), FUN=function(i) {
  # perfs <- sapply(seq(n.iters.per.paramset), FUN=function(i) {
  gp.model <- fit.model(pars, kernel.type)
  
  perf <- fit.model(gp.model, kernel.type, observed.x, observed.y)gh dfh fgf dhf hfd
  # return(perf)
  # })
  elapsed_time <- Sys.time() - start_time
}
